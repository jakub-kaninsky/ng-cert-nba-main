...
import {numbersOfDays, TeamFilter} from "./filter.models";
...
export class NbaService {
  ...
  trackedTeams: Team[] = [];
  numberOfDays$: BehaviorSubject<number> = new BehaviorSubject<number>(numbersOfDays[0]);
  ...
  removeTrackedTeam(index: number): void {
    // It is better to send index, otherwise it will incorrectly treat duplicates
  ...
  getLastResults(team: Team, numberOfDays = numbersOfDays[0] ): Observable<Game[]> {
  ...
  /**
   * Returns the observable of games using the stream of number of days
   * @param team
   */
  getGames(team: Team): Observable<Game[]> {
    return this.numberOfDays$.pipe(switchMap(numbersOfDays => {
      return this.getLastResults(team, numbersOfDays);
    }));
  }
  ...
  private getDaysQueryString(nbOfDays = numbersOfDays[0]): string {
    let qs = "";
